...
RSpec.describe Label, type: :model do
  ...
  let(:comment) { topic.comments.create!(title: RandomData.random_sentence, body: RandomData.random_paragraph, user: user) }
  ...
  it { is_expected.to have_many(:comments).through(:labelings) }

  describe "labelings" do
    it "allows the same label to be associated with a different topic, post, comment" do
      topic.labels << label
      post.labels << label
      ...
      comment_label = comment.labels[0]
# #11
      expect(topic_label).to eql(post_label).to eql(comment_label)
    end
  end
